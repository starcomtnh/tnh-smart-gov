...
public class AuthenticationService {
        ...
        public AuthenticationDTO login(AuthenticationForm authenticationForm, HttpServletRequest request) {
                ...
                var tenant = tenantService.findByNameAndActiveTrue(tenantName)
                                .orElseThrow(() -> new BWCNotFoundException(
                                                messageService.getMessage("tenant.not.found", tenantName)));
                ...
                try {
                        ...
                        var accessToken = jwtTokenService.generateToken(userDetails, request, sessionId, tenantName)
                                        .orElseThrow(() -> new BWCInvalidTokenException(
                                                        messageService.getMessage("jwt.token.invalid")));

                        var refreshToken = jwtTokenService
                                        .generateRefreshToken(userDetails, request, sessionId, tenantName)
                                        .orElseThrow(() -> new BWCInvalidTokenException(
                                                        messageService.getMessage("jwt.token.invalid")));
        ...
        public AuthenticationDTO refreshToken(HttpServletRequest request) {
                ...
                var tenantName = jwtTokenService.extractTenantId(refreshToken)
                                .orElseThrow(() -> new BWCInvalidTokenException(
                                                messageService.getMessage("jwt.token.invalid")));
                var tenant = tenantService.findByNameAndActiveTrue(tenantName)
                                .orElseThrow(() -> new BWCNotFoundException(
                ...
                try {
                        ...
                        if (!securityProperties.getJwt().isAllowMultipleDevices()) {
                                ...
                                privilegeCacheService.clearUserPrivilegeAsync(tenantName,
                                                String.valueOf(userDetails.getUser().getId()), sessionId);
                        } else {
                        ...
                        var newAccessToken = jwtTokenService
                                        .generateToken(userDetails, request, UUID.fromString(sessionId), tenantName)
                                        .orElseThrow(() -> new BWCInvalidTokenException(
                                                        messageService.getMessage("jwt.token.invalid")));

                        privilegeCacheService.clearUserCache(tenantName, String.valueOf(userDetails.getUser().getId()));
